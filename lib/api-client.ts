...
async function postJson<TReq, TRes>(path: `/api/${string}`, body: TReq): Promise<TRes> {
...
export function searchDeal(req: DealSearchRequest): Promise<DealSearchResult> {
  return postJson<DealSearchRequest, DealSearchResult>("/api/deals/search", req);
}
...
export function searchLeads(req: LeadSearchRequest): Promise<LeadSearchResult> {
  return postJson<LeadSearchRequest, LeadSearchResult>("/api/leads/search", req);
}
