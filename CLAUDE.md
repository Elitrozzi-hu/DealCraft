...
- `lib/llm/` — `generate` abstraction: `generate.ts` (thin dispatcher) + `registry.ts` (`getGenerationProvider()`) + `providers/` (`openrouter`, `glados`) + `types.ts` (`CommonOptions`/`GenerationUsage`/`GenerationProvider` contracts). `generations/<task>/` holds named LLM tasks: each has `prompt.ts` (exports `render*Prompt()`) + `structured-output.ts` (Zod schema). Current tasks: `company-research/`, `company-signals/`.
